...
def view_orders(request):
    ...
    elements = RestaurantMenuItem.objects.all()
    products_in_rests = elements.values('product', 'restaurant__name')
    orders = Order.objects.total_price().exclude(status='completed')\
        .order_by('status')
    for order in orders:
        for item in order.items.all():
            available_in = list(filter(
                lambda x: x['product'] == item.product.id, products_in_rests
            ))
            item.available_in = [d['restaurant__name'] for d in available_in]
        order.available_in = set.intersection(
            *map(set, [item.available_in for item in order.items.all()])
        )
        order_coord = (order.latitude, order.longitude)
        restaurants_with_distances = []
        for restaurant in restaurants:
            if restaurant.name in order.available_in:
